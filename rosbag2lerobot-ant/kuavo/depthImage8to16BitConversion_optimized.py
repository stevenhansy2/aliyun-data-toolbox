...
def compress_range(image, min_val = default_max_range/4, max_val = default_max_range*3/4):
    ...
    # 向量化处理
    compressed = (image * (target_range / original_range)) + min_val + np.random.randint(0, int(default_max_range * 0.2))
...
def smear_by_gray_blocks_optimized(gray_image, fill_image, n_clusters=8):
    ...
    scale = 2
    small_gray = cv2.resize(gray_image, (w // scale, h // scale), interpolation=cv2.INTER_NEAREST)
    small_fill = cv2.resize(fill_image, (w // scale, h // scale), interpolation=cv2.INTER_NEAREST)

    # 将灰度图像展平并转换为浮点数
    pixels = small_gray.reshape(-1, 1).astype(np.float32)

    # 使用 MiniBatchKMeans 进行聚类
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, random_state=0, batch_size=256, n_init=1)
    ...
    for label in range(n_clusters):
        ...
        if np.any(mask):
            block_value = max(np.mean(fill_image[mask]) * 0.8 - np.random.randint(0, 21), default_max_range * threshold_value)
...
def smear_by_color_blocks_optimized(color_image, fill_image, n_clusters=8):
    ...
    scale = 2
    small_color = cv2.resize(color_image, (w//scale, h//scale), interpolation=cv2.INTER_NEAREST)
    small_fill = cv2.resize(fill_image, (w//scale, h//scale), interpolation=cv2.INTER_NEAREST)

    pixels = small_color.reshape(-1, c).astype(np.float32)
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, random_state=0, batch_size=100, n_init=1)
    labels = kmeans.fit_predict(pixels)
    labels = labels.reshape(h//scale, w//scale)
