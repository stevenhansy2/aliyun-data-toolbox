#!/usr/bin/env python3
"""
合并多个 batch 数据为一个连续的 episode
"""

import os
import json
import shutil
import argparse
import subprocess
from pathlib import Path
from lerobot.datasets.utils import serialize_dict
import numpy as np
try:
    import pyarrow.parquet as pq
    import pyarrow as pa
except ImportError:
    print("请先安装 pyarrow: pip install pyarrow")
    exit(1)


def get_batch_dirs(base_path: Path) -> list:
    """获取所有 batch 目录，按名称排序"""
    batch_dirs = sorted([d for d in base_path.iterdir()
                        if d.is_dir() and d.name.startswith('batch_')])
    return batch_dirs


def merge_parquet_files(batch_dirs: list, output_dir: Path) -> int:
    """合并所有 parquet 文件，返回总帧数"""
    print("\n[1/4] 合并 Parquet 数据...")

    all_tables = []
    frame_offset = 0
    timestamp_offset = 0.0

    for batch_dir in batch_dirs:
        parquet_path = batch_dir / "data" / "chunk-000" / "episode_000000.parquet"
        if not parquet_path.exists():
            print(f"  警告: {parquet_path} 不存在，跳过")
            continue

        table = pq.read_table(parquet_path)
        num_rows = table.num_rows

        # 获取当前 batch 的最大时间戳（用于计算下一个 batch 的偏移）
        max_timestamp = 0.0
        if 'timestamp' in table.column_names:
            timestamps = table.column('timestamp').to_pylist()
            max_timestamp = max(timestamps) if timestamps else 0.0

        print(f"  {batch_dir.name}: {num_rows} 帧 (frame_offset: {frame_offset}, ts_offset: {timestamp_offset:.3f}s)")

        # 更新 frame_index 列
        if 'frame_index' in table.column_names:
            frame_indices = [i + frame_offset for i in range(num_rows)]
            table = table.set_column(
                table.schema.get_field_index('frame_index'),
                'frame_index',
                pa.array(frame_indices)
            )

        # 更新 index 列
        if 'index' in table.column_names:
            indices = [i + frame_offset for i in range(num_rows)]
            table = table.set_column(
                table.schema.get_field_index('index'),
                'index',
                pa.array(indices)
            )

        # 更新 timestamp 列（添加偏移使时间戳连续）
        if 'timestamp' in table.column_names:
            new_timestamps = [t + timestamp_offset for t in timestamps]
            table = table.set_column(
                table.schema.get_field_index('timestamp'),
                'timestamp',
                pa.array(new_timestamps, type=pa.float32())
            )

        all_tables.append(table)
        frame_offset += num_rows
        # 下一个 batch 的时间戳偏移 = 当前偏移 + 当前最大时间戳 + 一帧间隔(1/30s)
        timestamp_offset += max_timestamp + (1.0 / 30.0)

    # 合并所有表
    merged_table = pa.concat_tables(all_tables)

    # 保存合并后的 parquet
    output_data_dir = output_dir / "data" / "chunk-000"
    output_data_dir.mkdir(parents=True, exist_ok=True)
    output_parquet = output_data_dir / "episode_000000.parquet"
    pq.write_table(merged_table, output_parquet)

    print(f"  ✓ 合并完成: {merged_table.num_rows} 帧 -> {output_parquet}")
    return frame_offset


def concat_videos_ffmpeg(video_files: list, output_path: Path, temp_dir: Path) -> bool:
    """使用 ffmpeg 拼接视频文件"""
    if not video_files:
        return False

    # 创建临时文件列表
    list_file = temp_dir / f"_concat_list_{output_path.stem}.txt"
    with open(list_file, 'w') as f:
        for vf in video_files:
            f.write(f"file '{vf.absolute()}'\n")

    # 创建输出目录
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # 使用 ffmpeg 合并
    cmd = [
        'ffmpeg', '-y', '-f', 'concat', '-safe', '0',
        '-i', str(list_file),
        '-c', 'copy',
        str(output_path)
    ]

    try:
        subprocess.run(cmd, check=True, capture_output=True)
        return True
    except subprocess.CalledProcessError as e:
        print(f"    ✗ ffmpeg 错误: {e.stderr.decode()[:200]}")
        return False
    except FileNotFoundError:
        print("    ✗ 未找到 ffmpeg，请先安装: sudo apt install ffmpeg")
        return False
    finally:
        if list_file.exists():
            list_file.unlink()

def move_videos(batch_dirs: list, output_dir: Path, camera_names: list = None):
    """移动 videos 目录下的视频（不合并）"""
    print("\n[2/4] 移动 RGB 视频...")
    if camera_names is None:
        camera_names = ['head_cam_h',
                        'wrist_cam_l',
                        'wrist_cam_r']
    

    for camera in camera_names:
        print(f"  处理 {camera}...")

        for batch_dir in batch_dirs:
            video_path = batch_dir / "videos" / "chunk-000" / camera / "episode_000000.mp4"
            if video_path.exists():
                output_path = output_dir / "videos" / "chunk-000" / camera / f"{batch_dir.name}_episode_000000.mp4"
                output_path.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy(video_path, output_path)
                print(f"    ✓ 移动视频 -> {output_path.name}")

def merge_videos(batch_dirs: list, output_dir: Path, camera_names: list = None):
    """合并 videos 目录下的视频"""
    print("\n[2/4] 合并 RGB 视频...")
    if camera_names is None:
        camera_names = ['observation.images.head_cam_h',
                        'observation.images.wrist_cam_l',
                        'observation.images.wrist_cam_r']

    for camera in camera_names:
        print(f"  处理 {camera}...")

        video_files = []
        for batch_dir in batch_dirs:
            video_path = batch_dir / "videos" / "chunk-000" / camera / "episode_000000.mp4"
            if video_path.exists():
                video_files.append(video_path)

        if not video_files:
            print(f"    警告: 没有找到视频文件")
            continue

        output_path = output_dir / "videos" / "chunk-000" / camera / "episode_000000.mp4"
        if concat_videos_ffmpeg(video_files, output_path, output_dir):
            print(f"    ✓ 合并 {len(video_files)} 个视频 -> {output_path.name}")


def merge_depth_videos(batch_dirs: list, output_dir: Path):
    """合并 depth 目录下的深度视频"""
    print("\n[3/4] 合并深度视频...")

    depth_cameras = ['observation.videos.depth.head_cam_h',
                     'observation.videos.depth.wrist_cam_l',
                     'observation.videos.depth.wrist_cam_r']

    for camera in depth_cameras:
        print(f"  处理 {camera}...")

        video_files = []
        for batch_dir in batch_dirs:
            video_path = batch_dir / "depth" / "chunk-000" / camera / "episode_000000.mkv"
            if video_path.exists():
                video_files.append(video_path)

        if not video_files:
            print(f"    警告: 没有找到深度视频文件")
            continue

        output_path = output_dir / "depth" / "chunk-000" / camera / "episode_000000.mkv"
        if concat_videos_ffmpeg(video_files, output_path, output_dir):
            print(f"    ✓ 合并 {len(video_files)} 个视频 -> {output_path.name}")


def merge_metadata(batch_dirs: list, output_dir: Path, total_frames: int):
    """合并 metadata.json"""
    print("\n[4/4] 合并元数据...")
    
    # 读取第一个 batch 的 metadata 作为基础
    base_metadata = None
    all_actions = []
    frame_offset = 0
    
    for batch_dir in batch_dirs:
        metadata_path = batch_dir / "metadata.json"
        parquet_path = batch_dir / "data" / "chunk-000" / "episode_000000.parquet"
        
        if not metadata_path.exists():
            continue
        
        with open(metadata_path, 'r', encoding='utf-8') as f:
            metadata = json.load(f)
        
        if base_metadata is None:
            base_metadata = metadata.copy()
        
        # 获取当前 batch 的帧数
        num_frames = 0
        if parquet_path.exists():
            table = pq.read_table(parquet_path)
            num_frames = table.num_rows
        
        # 处理 action_config
        for action in metadata.get('label_info', {}).get('action_config', []):
            action_copy = action.copy()
            # 更新帧范围
            if action_copy.get('start_frame') is not None:
                action_copy['start_frame'] += frame_offset
            if action_copy.get('end_frame') is not None:
                action_copy['end_frame'] += frame_offset
            all_actions.append(action_copy)
        
        frame_offset += num_frames
    
    if base_metadata:
        # 合并并去重 action_config（基于 skill 和时间戳），同时优先保留有有效帧范围的标注
        # 如果同一个 (skill, timestamp_utc) 存在多次：
        #   - 如果已有的是 None-None，而新的有 start/end_frame，则用新的替换
        #   - 否则保持第一次出现的
        merged_actions = {}

        def _has_valid_frames(a: dict) -> bool:
            return a.get("start_frame") is not None and a.get("end_frame") is not None

        for action in all_actions:
            key = (action.get('skill'), action.get('timestamp_utc'))
            if key not in merged_actions:
                merged_actions[key] = action
            else:
                old = merged_actions[key]
                # 如果旧的是 None-None，新的是有效帧，则用新覆盖旧
                if (not _has_valid_frames(old)) and _has_valid_frames(action):
                    merged_actions[key] = action

        unique_actions = list(merged_actions.values())

        # ---- 将动作按时间排序，并尽量保证帧区间连续（与原始标注风格一致）----
        if unique_actions:
            # 1) 按 timestamp_utc 排序，保证时间顺序一致
            unique_actions.sort(key=lambda a: a.get("timestamp_utc") or "")

            # 2) 规范第一个动作的起止帧
            first = unique_actions[0]
            # 起始帧缺失则从 0 开始
            if first.get("start_frame") is None:
                first["start_frame"] = 0
            else:
                first["start_frame"] = max(
                    0, min(int(first["start_frame"]), total_frames - 1)
                )
            # 结束帧至少不小于起始帧，且不超过 total_frames-1
            if first.get("end_frame") is None:
                first["end_frame"] = first["start_frame"]
            else:
                first["end_frame"] = max(
                    first["start_frame"],
                    min(int(first["end_frame"]), total_frames - 1),
                )

            # 3) 后续动作：起点强制衔接上一个动作的 end_frame
            for i in range(1, len(unique_actions)):
                prev = unique_actions[i - 1]
                cur = unique_actions[i]

                prev_end = prev.get("end_frame")
                if prev_end is None:
                    prev_end = prev.get("start_frame", 0)
                prev_end = max(0, min(int(prev_end), total_frames - 1))

                cur["start_frame"] = prev_end
                if cur.get("end_frame") is None:
                    cur["end_frame"] = prev_end
                else:
                    cur["end_frame"] = max(
                        prev_end, min(int(cur["end_frame"]), total_frames - 1)
                    )

            # 4) 最后一个动作：结束帧拉到整段 episode 末尾
            last = unique_actions[-1]
            last["end_frame"] = total_frames - 1

        base_metadata['label_info']['action_config'] = unique_actions
        base_metadata['total_frames'] = total_frames
        
        # 保存
        output_metadata = output_dir / "metadata.json"
        with open(output_metadata, 'w', encoding='utf-8') as f:
            json.dump(base_metadata, f, ensure_ascii=False, indent=4)
        
        print(f"  ✓ 合并完成 -> {output_metadata}")
        print(f"  ✓ 共 {len(unique_actions)} 个动作标注")


def merge_meta_files(
    batch_dirs: list,
    output_dir: Path,
    total_frames: int,
    cam_stats: dict,
    video_height: int | None = None,
    video_width: int | None = None,
):
    """合并 meta 目录下的文件

    video_height / video_width: 实际保存视频的高和宽，用于 info.json 中相机特征的 shape 与 info。
    若不传则从第一个 batch 的 info.json 中读取；若仍无则默认 480 x 848。
    """
    print("\n[附加] 合并元数据文件...")

    output_meta_dir = output_dir / "meta"
    output_meta_dir.mkdir(parents=True, exist_ok=True)

    # 1. 合并 episodes.jsonl
    episodes_data = {"episode_index": 0, "tasks": ["DEBUG"], "task_label": "DEBUG",
                     "length": total_frames, "action_config": None}
    with open(output_meta_dir / "episodes.jsonl", 'w', encoding='utf-8') as f:
        f.write(json.dumps(episodes_data, ensure_ascii=False) + '\n')
    print(f"  ✓ episodes.jsonl")

    # 2. 合并 info.json（使用第一个 batch 的，更新帧数；相机 shape 使用真实保存视频高宽）
    first_info = None
    for batch_dir in batch_dirs:
        info_path = batch_dir / "meta" / "info.json"
        if info_path.exists():
            with open(info_path, 'r', encoding='utf-8') as f:
                first_info = json.load(f)
            break

    if first_info:
        first_info['total_frames'] = total_frames
        first_info['total_videos'] = 3
        for cam in cam_stats.keys():
            # 优先使用 cam_stats 中记录的实际视频高宽（来自首帧尺寸）
            stats_for_cam = cam_stats.get(cam, {})
            h = stats_for_cam.get("height")
            w = stats_for_cam.get("width")

            # 若 cam_stats 中无尺寸信息，则使用传入的 video_height / video_width
            if h is None:
                h = video_height
            if w is None:
                w = video_width

            # 仍为空则尝试从首个 batch 的 feature 中读取 shape 或 info
            if h is None or w is None:
                existing = first_info.get("features", {}).get(cam, {})
                shape = existing.get("shape")
                info_v = existing.get("info") or {}
                if shape and len(shape) >= 3:
                    if h is None:
                        h = int(shape[1])
                    if w is None:
                        w = int(shape[2])
                if h is None:
                    h = info_v.get("video.height")
                if w is None:
                    w = info_v.get("video.width")

            # 最后兜底：默认 480x848
            if h is None:
                h = 480
            if w is None:
                w = 848

            first_info["features"][cam] = {
                "dtype": "video",
                "shape": [3, h, w],
                "names": ["channels", "height", "width"],
                "info": {
                    "video.height": h,
                    "video.width": w,
                    "video.codec": "h264",
                    "video.pix_fmt": "yuv420p",
                    "video.is_depth_map": False,
                    "video.fps": 30,
                    "video.channels": 3,
                    "has_audio": False
                }
            }
        with open(output_meta_dir / "info.json", 'w', encoding='utf-8') as f:
            json.dump(first_info, f, ensure_ascii=False, indent=4)
        print(f"  ✓ info.json (total_frames: {total_frames})")

    # 3. 合并 tasks.jsonl
    tasks_path = batch_dirs[0] / "meta" / "tasks.jsonl"
    if tasks_path.exists():
        shutil.copy(tasks_path, output_meta_dir / "tasks.jsonl")
        print(f"  ✓ tasks.jsonl")

    # 4. 合并 episodes_stats.jsonl（聚合 min/max/mean/std；timestamp 按批次加偏移；count 仅 1 维）
    def _agg_init(shape_len: int):
        return {
            "min": None,
            "max": None,
            "mean": None,
            "std": None,
            # 聚合过程不累计按维 count，仅使用单一标量累计样本数，最终输出 count=[total_frames]
            "_sum": [0.0] * shape_len,      # Σ(x)
            "_sum_sq": [0.0] * shape_len,   # Σ(x^2)
            "_n": 0,                        # 样本总数（标量）
        }

    def _resize_acc(acc, new_len: int):
        """当遇到不同维度长度时，扩展/截断累加器"""
        def _res(arr, fill):
            if arr is None:
                return [fill] * new_len
            if len(arr) < new_len:
                return arr + [fill] * (new_len - len(arr))
            return arr[:new_len]
        acc["_sum"]    = _res(acc["_sum"], 0.0)
        acc["_sum_sq"] = _res(acc["_sum_sq"], 0.0)
        acc["min"]     = _res(acc["min"], float("inf"))
        acc["max"]     = _res(acc["max"], float("-inf"))
        acc["mean"]    = _res(acc["mean"], 0.0)
        acc["std"]     = _res(acc["std"], 0.0)

    def _as_list(v):
        if v is None:
            return None
        return v if isinstance(v, list) else [v]

    def _reduce_stats(acc, part, key_name: str):
        vals = part.get(key_name)
        if vals is None:
            return
        vals = _as_list(vals)

        # 动态对齐维度
        if len(acc["_sum"]) != len(vals):
            _resize_acc(acc, max(len(acc["_sum"]), len(vals)))

        if key_name == "min":
            if acc["min"] is None:
                acc["min"] = vals.copy()
            else:
                acc["min"] = [min(a, b) for a, b in zip(acc["min"], vals)]
        elif key_name == "max":
            if acc["max"] is None:
                acc["max"] = vals.copy()
            else:
                acc["max"] = [max(a, b) for a, b in zip(acc["max"], vals)]
        elif key_name == "mean":
            # 需要 count（单一标量或 [n]）
            part_count = part.get("count")
            if part_count is None:
                return
            n = _as_list(part_count)[0]  # count 只有 1 维
            # 累计 Σ(x) 与 n
            for i in range(len(vals)):
                m = float(vals[i])
                acc["_sum"][i] += m * n
            acc["_n"] += n
        elif key_name == "std":
            # 需要 mean 与 count
            part_count = part.get("count")
            part_mean = part.get("mean")
            if part_count is None or part_mean is None:
                return
            n = _as_list(part_count)[0]          # count 只有 1 维
            means = _as_list(part_mean)
            if len(means) != len(vals):
                # 对齐长度
                if len(means) < len(vals):
                    means = means + [0.0] * (len(vals) - len(means))
                else:
                    means = means[:len(vals)]
            # 累计 Σ(x^2) = (std^2 + mean^2) * n
            for i in range(len(vals)):
                s = float(vals[i])
                m = float(means[i])
                acc["_sum_sq"][i] += (s * s + m * m) * n
            # acc["_n"] 在 mean 阶段已累计，这里不重复加

    def _finalize(acc):
        n = acc["_n"]
        # 计算 mean/std
        if n <= 0:
            acc["mean"] = [0.0] * len(acc["_sum"])
            acc["std"] = [0.0] * len(acc["_sum"])
        else:
            means, stds = [], []
            for i in range(len(acc["_sum"])):
                m = acc["_sum"][i] / n
                var = max(acc["_sum_sq"][i] / n - m * m, 0.0)
                means.append(m)
                stds.append(var ** 0.5)
            acc["mean"] = means
            acc["std"] = stds
        # 清理临时项
        for k in ("_sum", "_sum_sq", "_n"):
            acc.pop(k, None)

    stats_agg = {"episode_index": 0, "stats": {}}
    h_count = 0
    for batch_idx, batch_dir in enumerate(batch_dirs):
        stats_path = batch_dir / "meta" / "episodes_stats.jsonl"
        if not stats_path.exists():
            continue
        try:
            with open(stats_path, 'r', encoding='utf-8') as f:
                line = f.readline().strip()
                if not line:
                    continue
                data = json.loads(line)
                part_stats = data.get("stats", {})
                count = h_count
                ts_offset = count / 30.0  # 每批时间偏移
                h_count = part_stats.get("timestamp", {}).get("count", [0])[0]

                for metric_name, metric_vals in part_stats.items():
                    # 复制并偏移 timestamp 的 min/max/mean
                    adj = dict(metric_vals)
                    if metric_name == "timestamp":
                        for kk in ("min", "max", "mean"):
                            if kk in adj and adj[kk] is not None:
                                v = adj[kk]
                                if isinstance(v, list):
                                    adj[kk] = [float(x) + ts_offset for x in v]
                                else:
                                    adj[kk] = float(v) + ts_offset

                    # 维度从 min/max/mean/std 中推断（跳过 count，因其只有 1 维）
                    sample_len = 1
                    for kk in ("min", "max", "mean", "std"):
                        v = adj.get(kk)
                        if v is not None:
                            sample_len = len(v) if isinstance(v, list) else 1
                            break

                    acc = stats_agg["stats"].get(metric_name)
                    if acc is None:
                        acc = _agg_init(sample_len)
                        stats_agg["stats"][metric_name] = acc
                    elif len(acc["_sum"]) != sample_len:
                        _resize_acc(acc, max(len(acc["_sum"]), sample_len))

                    # 聚合
                    _reduce_stats(acc, adj, "min")
                    _reduce_stats(acc, adj, "max")
                    _reduce_stats(acc, adj, "mean")
                    _reduce_stats(acc, adj, "std")
        except Exception as e:
            print(f"  警告: 读取 {stats_path} 失败: {e}")

    # 完成均值/方差计算，并统一输出 count=[total_frames]
    for metric_name, acc in stats_agg["stats"].items():
        _finalize(acc)
        acc["count"] = [int(total_frames)]  # 注意：count 只有 1 维
    # print(cam_stats)
    # for cam in cam_stats:
    #     cam["count"] = [int(total_frames)]
    # cam_stats = { k:v if v=="count" else k: {"count": np.array([int(total_frames)])} for k,v in cam_stats.items()}
    cam_stats = { k:{ k1:np.array([int(total_frames)]) if k1=="count" else v1 for k1,v1 in v.items()} for k,v in cam_stats.items()}
    stats_agg["stats"].update(serialize_dict(cam_stats))
    # stats_agg["stats"] = serialize_dict(stats_agg["stats"])
    # 写出聚合后的 episodes_stats.jsonl
    out_stats_path = output_meta_dir / "episodes_stats.jsonl"
    with open(out_stats_path, 'w', encoding='utf-8') as f:
        f.write(json.dumps(stats_agg, ensure_ascii=False) + "\n")
    print(f"  ✓ episodes_stats.jsonl（聚合 {len(batch_dirs)} 个 batch，count=[{int(total_frames)}]）")

    # 5. 合并 parameters 目录（仅复制第一个 batch 的）
    first_params_dir = batch_dirs[0] / "parameters"
    output_params_dir = output_dir / "parameters"
    if first_params_dir.exists():
        shutil.copytree(first_params_dir, output_params_dir, dirs_exist_ok=True)
        print(f"  ✓ parameters 目录 {output_params_dir}")

def main():
    parser = argparse.ArgumentParser(description='合并多个 batch 为一个连续的 episode')
    parser.add_argument('--input', '-i', type=str, default='.',
                        help='包含 batch 目录的输入路径 (默认: 当前目录)')
    parser.add_argument('--output', '-o', type=str, default='merged_batch',
                        help='输出目录名称 (默认: merged_batch)')
    parser.add_argument('--skip-video', action='store_true',
                        help='跳过视频合并（加快处理速度）')
    args = parser.parse_args()

    args.input = "/home/leju_kuavo/kuavo_cb/temp/Kuavo-Data-Tool-Box-test-memory/Kuavo-Data-Tool-Box-test-memory/rosbag2lerobot/testoutput/28b7d077-70a8-491e-a21e-250da1e8b0af"
    args.skip_video = True


    base_path = Path(args.input).resolve()
    output_dir = base_path / args.output

    print(f"输入路径: {base_path}")
    print(f"输出路径: {output_dir}")

    # 获取所有 batch 目录
    batch_dirs = get_batch_dirs(base_path)
    if not batch_dirs:
        print("错误: 未找到 batch 目录")
        return

    print(f"找到 {len(batch_dirs)} 个 batch 目录:")
    for d in batch_dirs:
        print(f"  - {d.name}")

    # 创建输出目录
    output_dir.mkdir(parents=True, exist_ok=True)

    # 合并 parquet 数据
    total_frames = merge_parquet_files(batch_dirs, output_dir)

    # 合并 RGB 视频
    if not args.skip_video:
        merge_videos(batch_dirs, output_dir)
        merge_depth_videos(batch_dirs, output_dir)
    else:
        print("\n[2/4] 跳过视频合并")
        print("\n[3/4] 跳过深度视频合并")

    # 合并 metadata.json
    merge_metadata(batch_dirs, output_dir, total_frames)

    # 合并 meta 目录（独立脚本无 cam_stats，传空 dict；相机尺寸从 batch 的 info 读取）
    merge_meta_files(batch_dirs, output_dir, total_frames, {})

    # 创建空的 images 目录结构（与原始数据一致）
    for cam in ['observation.images.head_cam_h',
                'observation.images.wrist_cam_l',
                'observation.images.wrist_cam_r']:
        (output_dir / "videos" / "chunk-000" / cam).mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 50)
    print("✓ 合并完成!")
    print(f"  输出目录: {output_dir}")
    print(f"  总帧数: {total_frames}")


if __name__ == '__main__':
    main()

