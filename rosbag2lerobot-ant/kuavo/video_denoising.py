...
def repair_depth_noise_focused(depth_img, max_valid_depth=10000, median_kernel=5, 
                              detect_white_spots=True, spot_size_range=(10, 1000)):
    """
    专门针对黑色背景下白色圆斑噪点的修复算法（16位原生处理）
    """
    # print(f"[DEBUG] 开始检测白色圆斑噪点，图像形状: {depth_img.shape}")
    # print(f"[DEBUG] 深度值范围: {depth_img.min()} - {depth_img.max()}")
    # print(f"[DEBUG] 图像数据类型: {depth_img.dtype}")
    ...
    if detect_white_spots:
        ...
        if len(valid_depths) > 100:  # 确保有足够的有效像素
            ...
            # 设置阈值：比平均值高2个标准差的区域
            high_depth_threshold = mean_depth + 1 * std_depth
            #print(f"[DEBUG] 深度统计: 均值={mean_depth:.1f}, 标准差={std_depth:.1f}, 高值阈值={high_depth_threshold:.1f}")
            ...
            for i in range(1, num_labels):  # 跳过背景标签0
                ...
                # 检查是否符合白色圆斑特征
                if spot_size_range[0] <= area <= spot_size_range[1]:     # 接近圆形（放宽一点）
        ...
        if len(valid_depths) >= 0:  # 确保有有效深度数据
            ...
            neighborhood_avg = cv2.filter2D(depth_img.astype(np.float32), -1, kernel_avg)
            
            # 找出比邻域平均值高很多的点
            significant_peaks = local_maxima & (depth_img > neighborhood_avg + std_depth)
            
            # 对这些峰值点进行连通域分析
            if np.sum(significant_peaks) > 0:
                # 修正：正确解包cv2.connectedComponents的返回值
                peak_num, peak_labels = cv2.connectedComponents(significant_peaks.astype(np.uint8))
    ...
    # print(f"[DEBUG] 距离噪点: {distance_noise_count} 像素")
    # print(f"[DEBUG] 白色圆斑噪点: {white_spot_count} 像素")
    # print(f"[DEBUG] 总噪点: {total_noise_count} 像素")
    ...
    if white_spot_count > 0:
        # 找出每个独立的白色圆斑区域
        spot_num, spot_labels = cv2.connectedComponents(white_spot_mask.astype(np.uint8))
        ...
        for spot_id in range(1, spot_num):
            ...
            expanded_region = cv2.dilate(spot_region.astype(np.uint8), dilate_kernel, iterations=2)
    ...
    if np.sum(distance_only_mask) > 0:
        ...
        #print(f"[DEBUG] 距离噪点使用 {median_kernel}x{median_kernel} 中值滤波修复")
    
    #print(f"[DEBUG] 16位原生修复完成，无精度损失")
    
    return repaired_img


def visualize_repair_comparison_enhanced(original_img, repaired_img, noise_mask, save_path=None):
    """
    增强版可视化，突出显示白色圆斑检测
    """
    # 创建彩色深度图
    ...
    if np.sum(noise_mask) > 0:
        ...
        y_min, y_max = max(0, noise_coords[0].min()-20), min(original_img.shape[0], noise_coords[0].max()+20)
        x_min, x_max = max(0, noise_coords[1].min()-20), min(original_img.shape[1], noise_coords[1].max()+20)
    ...
    if save_path:
        ...
        print(f"[INFO] Enhanced comparison saved to: {save_path}")
...
def create_depth_colormap(depth_img):
    ...
    if max_depth == min_depth:
        ...
        depth_normalized = (depth_img.astype(np.float32) - min_depth) / (max_depth - min_depth)
        depth_normalized = np.clip(depth_normalized, 0, 1)
    
    # 创建RGB图像
    colored_depth = np.zeros((depth_img.shape[0], depth_img.shape[1], 3), dtype=np.uint8)
    
    # 只对有效像素（非零）进行着色
    valid_mask = depth_img > 0
    colored_depth[valid_mask, 0] = (255 * (1 - depth_normalized[valid_mask])).astype(np.uint8)  # Red
    colored_depth[valid_mask, 2] = (255 * depth_normalized[valid_mask]).astype(np.uint8)       # Blue
...
def test_white_spot_repair(input_png_path):
    ...
    """
    print("=" * 70)
    print("开始白色圆斑噪点检测与修复测试（16位原生处理）")
    print("=" * 70)
    
    # 读取图像
    depth_img = cv2.imread(input_png_path, cv2.IMREAD_UNCHANGED)
    if depth_img is None:
        print(f"[ERROR] 无法读取文件: {input_png_path}")
        return
    ...
    print(f"修复耗时: {endtime - starttime}")
    # 保存修复后的图像
    ...
    visualize_repair_comparison_enhanced(depth_img, repaired_img, noise_mask, comparison_path)
    
    repaired_pixels = np.sum(noise_mask)
    print(f"✅ 白色圆斑修复成功！修复了 {repaired_pixels} 个噪点像素")
    print(f"💾 修复图像已保存到: {output_path}")
    print(f"📊 增强对比图已显示并保存")
