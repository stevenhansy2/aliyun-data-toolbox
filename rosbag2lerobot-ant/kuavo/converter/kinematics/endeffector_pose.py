...
def _resolve_urdf_path(urdf_path: str) -> str:
    """Resolve URDF path robustly regardless of current working directory."""
    if os.path.exists(urdf_path):
        return urdf_path

    base_dir = os.path.dirname(os.path.abspath(__file__))
    kuavo_dir = os.path.dirname(os.path.dirname(base_dir))
    candidates = [
        os.path.join(base_dir, urdf_path),
        os.path.join(base_dir, os.path.basename(urdf_path)),
        os.path.join(kuavo_dir, urdf_path),
        os.path.join(kuavo_dir, "assets", "urdf", os.path.basename(urdf_path)),
    ]
...
class KuavoPoseCalculator:
    def __init__(self, urdf_path):
        urdf_path = _resolve_urdf_path(urdf_path)
        self.plant = MultibodyPlant(0.0)
        parser = Parser(self.plant)
        # Drake API differs across versions; support common variants.
        if hasattr(parser, "AddModelFromFile"):
            parser.AddModelFromFile(urdf_path)
        elif hasattr(parser, "AddModelsFromFile"):
            parser.AddModelsFromFile(urdf_path)
        elif hasattr(parser, "AddModelsFromUrl"):
            url = "file://" + os.path.abspath(urdf_path)
            parser.AddModelsFromUrl(url)
        else:
            raise AttributeError(
                "Parser does not support AddModelFromFile/AddModelsFromFile/AddModelsFromUrl"
            )
...
def extract_pose_components(rigid_transform):
    ...
    return {
        ...
        'quaternion_xyzw': np.array([quaternion.x(), quaternion.y(), quaternion.z(), quaternion.w()], dtype=np.float32),
    }
...
def extract_and_format_eef_extrinsics(sensors_data_raw, urdf_path="./kuavo/assets/urdf/biped_s49.urdf"):
    """
...
if __name__ == "__main__":
    ...
    print("positions shape:", positions.shape)      # (N, 2, 3)
    print("quaternions shape:", quaternions.shape)  # (N, 2, 4)
