...
def encode_complete_videos_from_temp(
    ...
    # 防止个别编码进程偶发卡死导致主流程无法退出
    # 可通过环境变量 VIDEO_PROCESS_TIMEOUT_SEC 调整（默认 600 秒）
    try:
        process_timeout_sec = int(os.environ.get("VIDEO_PROCESS_TIMEOUT_SEC", "600"))
    except Exception:
    ...
    # 等待所有子进程完成（带超时，避免偶发卡死）
    def _join_with_timeout(proc_list, kind: str):
        for p in proc_list:
            p.join(timeout=process_timeout_sec)
            if p.is_alive():
                print(
                    f"[VIDEO][{kind}] ⚠️ 子进程 pid={p.pid} 超时 {process_timeout_sec}s，执行 terminate"
                )
                p.terminate()
                p.join(timeout=10)
                if p.is_alive():
                    print(
                        f"[VIDEO][{kind}] ⚠️ 子进程 pid={p.pid} terminate 后仍存活，执行 kill"
                    )
                    p.kill()
                    p.join(timeout=5)
            if p.exitcode not in (0, None):
                print(f"[VIDEO][{kind}] ⚠️ 子进程 pid={p.pid} exitcode={p.exitcode}")

    _join_with_timeout(color_procs, "COLOR")
    _join_with_timeout(depth_procs, "DEPTH")
