...
class ChunkedRosbagProcessor:
    ...
    def __init__(self, msg_processer, topic_process_map: dict,
        ...
        self.crop_range = crop_range  # 裁剪范围 (min_position, max_position), None 表示不裁剪
        self._resolved_topic_process_map = None

    def _resolve_topic_process_map(self, topic_info: dict) -> dict:
        resolved_map = {}
        for key, config in self._topic_process_map.items():
            candidates = config.get("topic_candidates", [config["topic"]])
            resolved_topic = next((topic for topic in candidates if topic in topic_info), config["topic"])
            resolved_map[key] = dict(config)
            resolved_map[key]["topic"] = resolved_topic
            resolved_map[key]["resolved"] = resolved_topic in topic_info
        return resolved_map

    def _active_topic_process_map(self) -> dict:
        return self._resolved_topic_process_map or self._topic_process_map
    ...
    def scan_timestamps_only(self, bag_file: str) -> Tuple[str, List[float], Dict[str, List[float]]]:
        ...
        self._resolved_topic_process_map = self._resolve_topic_process_map(topic_info)

        print(f"[Rosbag Topic Check] {bag_file}:")
        for topic_name in topic_info:
            print(f"  - {topic_name}")

        required_keys = ["action.kuavo_arm_traj", "observation.state", "action"] + list(self.camera_names)
        missing_keys = [
            key for key in required_keys
            if key in self._resolved_topic_process_map and not self._resolved_topic_process_map[key].get("resolved")
        ]
        if missing_keys:
            print(f"[Rosbag Topic Warning] Missing logical inputs: {missing_keys}")
            print("[Rosbag Check] Required topics missing, aborting.")
            import sys; sys.exit(1)
        else:
            print("[Rosbag Topic Check] All required logical inputs are present.")

        camera_topics = [
            self._resolved_topic_process_map[camera]["topic"]
            for camera in self.camera_names
            if camera in self._resolved_topic_process_map and self._resolved_topic_process_map[camera].get("resolved")
        ]
        for cam_topic in camera_topics:
            if cam_topic in topic_info:
                times = [t.to_sec() for _, _, t in bag.read_messages(topics=[cam_topic])]
                if len(times) > 1:
                    duration = times[-1] - times[0]
                    fps = len(times) / duration if duration > 0 else 0
                    if fps > 20:
                        print(f"[Rosbag Camera FPS] {cam_topic}: fps = {fps:.2f} (>20Hz) [OK]")
                    else:
                        print(f"[Rosbag Camera FPS Warning] {cam_topic}: fps = {fps:.2f} (<=20Hz) [TOO LOW!]")
                        print("[Rosbag Check] Camera fps too low, aborting.")
                        import sys; sys.exit(1)
                else:
                    print(f"[Rosbag Camera FPS Warning] {cam_topic}: insufficient frames")
                    print("[Rosbag Check] Camera topic insufficient frames, aborting.")
                    import sys; sys.exit(1)
            else:
                print(f"[Rosbag Camera FPS Warning] {cam_topic}: topic not found")
                print("[Rosbag Check] Camera topic missing, aborting.")
                import sys; sys.exit(1)
        # 统计每个话题的时间戳（不加载消息内容）
        all_timestamps = defaultdict(list)
        active_map = self._active_topic_process_map()
        topic_to_key = {}
        for k, v in active_map.items():
            if not v.get("resolved", True):
                continue
            topic_to_key.setdefault(v["topic"], []).append(k)

        _log("INFO", f"[Phase 1] Scanning timestamps from {bag_file}...")

        # 一次遍历，收集所有话题的时间戳
        all_topics = sorted(topic_to_key.keys())
        ...
        _log("INFO", f"Main timeline: {main_timeline} ({camera_counts[main_timeline]} frames)")
        ...
        if self.crop_range is not None:
            ...
            _log("INFO", f"Applied crop range {min_pos:.3f}-{max_pos:.3f}, cropped to frames {crop_start_idx}-{crop_end_idx} (remaining {len(raw_timestamps)} frames)")
        ...
        _log("INFO", f"Generated {len(main_timestamps)} aligned timestamps (from {len(raw_timestamps)} raw frames, dropped {self.sample_drop} frames at each end, jump={jump})")
    ...
    def process_in_chunks(
        ...
        _log("INFO", f"[Phase 2] Processing {len(main_timestamps)} frames in {num_chunks} chunks...")
        ...
        for chunk_idx in range(num_chunks):
            ...
            _log("DEBUG", f"Chunk {chunk_idx+1}/{num_chunks}: frames {start_idx}-{end_idx-1}, time range [{chunk_start_time:.3f}, {chunk_end_time:.3f}]")

            # 读取该时间范围内的消息
            _log("INFO", f"[Chunk {chunk_idx + 1}/{num_chunks}] reading messages for {len(chunk_timestamps)} frames in [{chunk_start_time:.3f}, {chunk_end_time:.3f}]")
            chunk_data = self._read_chunk_data(bag, chunk_start_time, chunk_end_time)
            chunk_timestamp_arrays = {
                key: np.array(sorted(data.keys()), dtype=np.float64)
                for key, data in chunk_data.items()
            }
            _log("INFO", f"[Chunk {chunk_idx + 1}/{num_chunks}] loaded {sum(len(v) for v in chunk_data.values())} messages across {len(chunk_data)} keys")
            ...
            for local_idx, (global_idx, main_stamp) in enumerate(
                ...
                aligned_frame = self._align_single_frame(
                    ...
                    chunk_timestamp_arrays=chunk_timestamp_arrays,
            ...
            if save_callback:
                save_callback()
                _log("INFO", f"Chunk {chunk_idx+1}/{num_chunks} processed and saved. Frames: {start_idx}-{end_idx-1}")

        bag.close()
        _log("INFO", f"Total frames processed: {total_frames}")
        return total_frames
    ...
    def _detect_arm_traj_gaps(self, all_timestamps: Dict[str, List[float]]) -> List[Tuple[float, float]]:
        ...
        if len(gaps) > 0:
            _log("INFO", f"Detected {len(gaps)} gaps in action.kuavo_arm_traj")
        return gaps
    ...
    def _read_chunk_data(self, bag: rosbag.Bag, start_time: float, end_time: float) -> Dict[str, Dict[float, dict]]:
        ...
        try:
            ...
            for topic, msg, t in bag.read_messages(
                ...
                keys = topic_to_key.get(topic)
                for key in keys:
                    msg_process_fn = active_map[key]["msg_process_fn"]
                    msg_data = msg_process_fn(msg)
                    msg_data["timestamp"] = t.to_sec()
                    chunk_data[key][t.to_sec()] = msg_data

        except Exception as e:
            _log("WARN", f"Time-range filtering failed: {e}, falling back to full scan")
            # 回退到全量扫描+手动过滤
    ...
    def _align_single_frame(
        ...
        chunk_timestamp_arrays: Dict[str, np.ndarray],
        timestamp_arrays: Dict[str, np.ndarray],
        ...
        for key in self._active_topic_process_map().keys():
            ...
            if key in chunk_data:
                key_chunk_data = chunk_data[key]
                if target_ts in key_chunk_data:
                    aligned_frame[key] = key_chunk_data[target_ts]
                    continue

                ts_array = chunk_timestamp_arrays.get(key)
                if ts_array is not None and len(ts_array) > 0:
                    idx = bisect.bisect_left(ts_array, target_ts)
                    if idx == 0:
                        closest_chunk_ts = float(ts_array[0])
                    elif idx == len(ts_array):
                        closest_chunk_ts = float(ts_array[-1])
                    else:
                        left_ts = float(ts_array[idx - 1])
                        right_ts = float(ts_array[idx])
                        closest_chunk_ts = right_ts if abs(right_ts - target_ts) < abs(left_ts - target_ts) else left_ts
                    aligned_frame[key] = key_chunk_data.get(closest_chunk_ts)
                else:
                    aligned_frame[key] = None
            else:
                aligned_frame[key] = None
    ...
    def _load_bag(self, bag_file: str) -> rosbag.Bag:
        ...
        try:
            ...
            _log("WARN", f"Bag file {bag_file} is unindexed, attempting to reindex...")
