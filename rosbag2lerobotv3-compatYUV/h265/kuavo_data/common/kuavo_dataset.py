...
from converter.reader.topic_map_builder import build_main_topic_process_map
...
def init_parameters(cfg):
    ...
    global TASK_DESCRIPTION, DEFAULT_CAMERA_TOPIC_SPECS, DEFAULT_SOURCE_TOPICS
    ...
    DEFAULT_CAMERA_TOPIC_SPECS = config.camera_topic_specs
    DEFAULT_SOURCE_TOPICS = config.source_topics
...
class KuavoMsgProcesser:
    ...
    @staticmethod
    def _looks_like_h26x(payload: bytes) -> bool:
    ...
    @staticmethod
    def _decode_h26x_frame(payload: bytes) -> np.ndarray:
        cmd = [
            'ffmpeg',
            '-loglevel', 'error',
            '-i', 'pipe:0',
        ...
        if result.returncode != 0 or not result.stdout:
            ...
            raise ValueError(f'Failed to decode H26x frame with ffmpeg: {err or "empty output"}')

        img_arr = np.frombuffer(result.stdout, dtype=np.uint8)
        bgr = cv2.imdecode(img_arr, cv2.IMREAD_COLOR)
        if bgr is None:
            raise ValueError('ffmpeg produced undecodable image data')
        return cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)

    @staticmethod
    def _decode_raw_image(msg) -> np.ndarray:
        ...
        yuyv_encodings = {'yuv422', 'yuv422_yuy2', 'yuyv', 'yuyv422'}
        if encoding in yuyv_encodings:
            yuv = raw.reshape(msg.height, msg.width, 2)
            return cv2.cvtColor(yuv, cv2.COLOR_YUV2RGB_YUY2)

        uyvy_encodings = {'uyvy', 'uyvy422'}
        if encoding in uyvy_encodings:
            yuv = raw.reshape(msg.height, msg.width, 2)
            return cv2.cvtColor(yuv, cv2.COLOR_YUV2RGB_UYVY)

        if encoding == 'nv12':
            yuv = raw.reshape(msg.height * 3 // 2, msg.width)
            return cv2.cvtColor(yuv, cv2.COLOR_YUV2RGB_NV12)

        if encoding == 'nv21':
            yuv = raw.reshape(msg.height * 3 // 2, msg.width)
            return cv2.cvtColor(yuv, cv2.COLOR_YUV2RGB_NV21)

        if encoding in {'yuv420', 'i420', 'yuv420p'}:
            yuv = raw.reshape(msg.height * 3 // 2, msg.width)
    ...
    def process_color_image(msg):
        """Process color frames from raw Image, JPEG-compressed, or H.265-compressed topics."""
        result = {"timestamp": msg.header.stamp.to_sec()}
        if hasattr(msg, 'encoding'):
            encoding = (msg.encoding or '').lower()
            cv_img = KuavoMsgProcesser._decode_raw_image(msg)
            yuv_pix_fmts = {
                'yuv422': 'yuyv422',
                'yuv422_yuy2': 'yuyv422',
                'yuyv': 'yuyv422',
            ...
            if encoding in yuv_pix_fmts:
                result['direct_video'] = {
                    'kind': 'rawvideo',
                    'payload': bytes(msg.data),
                    'width': int(msg.width),
                    'height': int(msg.height),
                    'pix_fmt': yuv_pix_fmts[encoding],
                }
        else:
            payload = bytes(msg.data)
            fmt = str(getattr(msg, 'format', '') or '').lower()
            if 'h265' in fmt or 'hevc' in fmt or KuavoMsgProcesser._looks_like_h26x(payload):
                cv_img = np.zeros((RESIZE_H, RESIZE_W, 3), dtype=np.uint8)
                result['direct_video'] = {
                    'kind': 'h265',
                    'payload': payload,
                }
                ...
                cv_img = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
        cv_img = KuavoMsgProcesser._resize_rgb(cv_img)
        result['data'] = cv_img
        return result
    ...
    def process_depth_image(msg):
        ...
        fmt = str(getattr(msg, 'format', '') or '').lower()
        if 'h265' in fmt or 'hevc' in fmt or KuavoMsgProcesser._looks_like_h26x(payload):
            rgb = KuavoMsgProcesser._decode_h26x_frame(payload)
            gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
            depth_image = cv2.resize(gray, (RESIZE_W, RESIZE_H), interpolation=cv2.INTER_NEAREST)
            return {"data": depth_image, "timestamp": msg.header.stamp.to_sec()}

        print(f'Unsupported depth payload format: {fmt}')
        return None
    ...
    @staticmethod
    def process_primary_hand_state(msg):
        if hasattr(msg, 'position'):
            return {"data": list(msg.position), "timestamp": msg.header.stamp.to_sec()}
        if hasattr(msg, 'left_hand_position') and hasattr(msg, 'right_hand_position'):
            state = list(msg.left_hand_position)
            state.extend(list(msg.right_hand_position))
            return {"data": state, "timestamp": msg.header.stamp.to_sec()}
        raise ValueError(f'Unsupported hand state message type: {type(msg)!r}')


    @staticmethod
    def process_camera_info(msg):
        distortion_model = getattr(msg, 'distortion_model', '')
        D = np.array(getattr(msg, 'D', []), dtype=np.float32)
        K = np.array(getattr(msg, 'K', []), dtype=np.float32)
        R = np.array(getattr(msg, 'R', []), dtype=np.float32)
        P = np.array(getattr(msg, 'P', []), dtype=np.float32)
        camera_vec = np.concatenate([D.ravel(), K.ravel(), R.ravel(), P.ravel()]).astype(np.float32)
        return {
            "data": camera_vec,
...
class KuavoRosbagReader:
    def __init__(self):
        ...
        self._topic_process_map = build_main_topic_process_map(
            self._msg_processer,
            DEFAULT_SOURCE_TOPICS,
        )
        self._topic_process_map.update(
            build_camera_topic_process_map(
