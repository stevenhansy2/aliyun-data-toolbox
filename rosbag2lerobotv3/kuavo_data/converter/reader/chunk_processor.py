...
class ChunkedRosbagProcessor:
    ...
    def scan_timestamps_only(self, bag_file: str) -> Tuple[str, List[float], Dict[str, List[float]]]:
        ...
        missing_topics = [t for t in required_topics if t not in topic_info]
        if missing_topics:
            print(f"[Rosbag Topic Warning] Missing topics: {missing_topics}")
            print("[Rosbag Check] Required topics missing, aborting.")
            import sys; sys.exit(1)
        else:
            print("[Rosbag Topic Check] All required topics are present.")
        # 帧率检测（检测相机话题帧率是否大于20Hz）
        camera_topics = [
            "/cam_h/color/image_raw/compressed",
            "/cam_r/color/image_raw/compressed",
            "/cam_l/color/image_raw/compressed",
