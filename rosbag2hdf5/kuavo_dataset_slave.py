...
class KuavoRosbagReader:
    ...
    def process_rosbag(
        ...
        for key, topic_info in self._topic_process_map.items():
            ...
            if (
                ...
                print(
                    f"[WARN] {topic} 未读取到数据，尝试降级到压缩深度话题 {topic_info['fallback_topic']}"
                )
                fallback_count = 0
                for _, msg, t in bag.read_messages(
                    topics=topic_info["fallback_topic"],
                    start_time=rospy.Time.from_sec(abs_start),
                    end_time=rospy.Time.from_sec(abs_end),
                ):
        ...
        if joint_q_items:
            ...
            positions, quaternions = extract_and_format_eef_extrinsics(
                [{"joint_q": q} for q in joint_q_list], urdf_path="./biped_s49.urdf"
            )
            # 组装为 [{data: ..., timestamp: ...}, ...]
            data["end.position"] = [
        ...
        return self.align_frame_data_optimized(
            data,
            drop_head,
            drop_tail,
            action_config=action_config,
            min_duration=min_duration,
        ) if is_align else data
    ...
    def align_frame_data_optimized(
        ...
        for key, data_list in preprocessed_data.items():
            ...
            if max_diff > 20:
                print(f"警告: {key} 时间戳对齐偏差过大: {max_diff:.1f}ms")
        # === 新增步骤8: 静止区域检测和裁剪 ===
        print("步骤8: 静止区域检测和裁剪（暂时跳过）")
        ...
        aligned_data, main_img_timestamps = self._adjust_frame_rate_to_30fps(
    ...
    def _final_alignment_validation(
        ...
        # 验证长度要求（至少150帧）
        min_required_frames = int(min_duration * 30)
        ...
        # 验证时间跨度要求（至少10秒）
        min_required_duration = float(min_duration)  # 秒
        if len(main_timestamps) > 1:
            time_span = main_timestamps[-1] - main_timestamps[0]
            ...
            for key, value in data_dict.items():
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        ...
                        print(f"创建数据集: {full_path}, 形状: {np.array(value).shape if value is not None else (0,)}, 类型提示: {dtype_hint}")
                    except Exception as e:
            ...
            if "state" in f:
                ...
                if "joint" in state_group:
                    ...
                    if "current_value" not in joint_group:
                        ...
                        print(f"添加缺失字段: state/joint/current_value (使用NaN表示缺失)")
