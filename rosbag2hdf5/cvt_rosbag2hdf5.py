...
def merge_metadata_and_moment(
    ...
    for field, desc in required_metadata_fields.items():
        ...
        if not value or (isinstance(value, str) and value.strip() == ""):
            error_msg = f"❌ metadata.json 缺失关键字段 '{field}' ({desc})，数据不完整"
    ...
    if not (task_group_name or task_name):
        error_msg = f"❌ metadata.json 缺失任务名称字段 'task_group_name' 和 'task_name' 都为空，数据不完整"
        print(f"[ERROR] {error_msg}")
        # raise ValueError(error_msg)

    if not (task_group_code or task_code):
        error_msg = f"❌ metadata.json 缺失任务编码字段 'task_group_code' 和 'task_code' 都为空，数据不完整"
        print(f"[ERROR] {error_msg}")
        # raise ValueError(error_msg)
    ...
    if not moments:
        error_msg = (
            f"❌ moment.json 中未找到有效的 moments 数据，标注信息缺失"
        )
        print(f"[ERROR] {error_msg}")
        # raise ValueError(error_msg)
    ...
    for i, m in enumerate(moments):
        ...
        # 验证时间戳
        if not trigger_time:
            error_msg = f"❌ moment.json 第{i+1}个动作缺失 'triggerTime' 时间戳，标注信息不完整"
            print(f"[ERROR] {error_msg}")
            # raise ValueError(error_msg)
        ...
        for field, desc in required_moment_fields.items():
            ...
            if not value:
                error_msg = f"❌ moment.json 第{i+1}个动作缺失 '{field}' ({desc})，标注信息不完整"
                print(f"[ERROR] {error_msg}")
                # raise ValueError(error_msg)
    ...
    for field, desc in final_required_fields.items():
        ...
        if not value or (isinstance(value, str) and value.strip() == ""):
            error_msg = f"❌ 转换后字段 '{field}' ({desc}) 为空，数据不完整"
            print(f"[ERROR] {error_msg}")
            # raise ValueError(error_msg)
    ...
    for m in moments:
        ...
        # 新增：验证动作配置中的关键字段不能为空
        if not skill_value:
            error_msg = f"❌ 动作配置中 skill 字段为空，动作标注不完整"
            print(f"[ERROR] {error_msg}")
            # raise ValueError(error_msg)

        if not action_text_value:
            error_msg = f"❌ 动作配置中 action_text 字段为空，动作标注不完整"
            print(f"[ERROR] {error_msg}")
            # raise ValueError(error_msg)

        if not english_action_text_value:
            error_msg = (
                f"❌ 动作配置中 english_action_text 字段为空，动作标注不完整"
            )
            print(f"[ERROR] {error_msg}")
            # raise ValueError(error_msg)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="request.json", help="Path to config YAML file"
    )
