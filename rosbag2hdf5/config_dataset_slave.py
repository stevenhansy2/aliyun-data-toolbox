...
def load_config_from_json(config_path: str) -> Config:
    ...
    for bag in config_dict.get('bags', []):
        ...
        if steps_start is not None and steps_end is not None:
            ...
            print(f"[CONFIG] Bag使用steps时间范围: start={steps_start}, end={steps_end} (从 {len(steps)} 个步骤中获取)")
        else:
            ...
            if steps:
                print(f"[CONFIG] Bag的steps中未找到有效时间范围，使用原始时间范围: start={original_start}, end={original_end}")
            else:
                print(f"[CONFIG] Bag无steps，使用原始时间范围: start={original_start}, end={original_end}")
