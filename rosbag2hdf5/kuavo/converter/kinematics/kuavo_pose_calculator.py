    ...
    with rosbag.Bag(bag_path, 'r') as bag:
        for topic, msg, t in bag.read_messages(topics=['/sensors_data_raw']):
            ...
            try:
                ...
                log_print(f"跳过时间戳 {t_sec} 的数据，原因: {e}")
                continue
    return camera_poses
    ...
    with rosbag.Bag(bag_path, 'r') as bag:
        for topic, msg, t in bag.read_messages(topics=['/sensors_data_raw']):
            ...
            try:
                ...
                # Debug
                # log_print("joint_q:", joint_q)
                # log_print("left_arm_q:", left_arm_q)
                # log_print("right_arm_q:", right_arm_q)
                # log_print("head_q:", head_q)
                # log_print("head_camera_pose:", head_camera_pose)
                # log_print("left_hand_camera_pose:", left_hand_camera_pose)
                # log_print("right_hand_camera_pose:", right_hand_camera_pose)
                # return 
                ...
                log_print(f"跳过时间戳 {t.to_sec()} 的数据，原因: {e}")
                continue
...
class KuavoPoseCalculator:
    def __init__(self, urdf_path):
        if _PYDRAKE_IMPORT_ERROR is not None:
            raise ImportError(
                "pydrake 导入失败。若报错包含 libmosek executable stack，"
                "请使用已修复的 Docker 镜像（构建阶段会自动 clear-execstack）。"
            ) from _PYDRAKE_IMPORT_ERROR
...
if __name__ == "__main__":
    ...
    # log_print("URDF path:", urdf_path)
    urdf_path = "./assets/urdf/biped_s45.urdf"
