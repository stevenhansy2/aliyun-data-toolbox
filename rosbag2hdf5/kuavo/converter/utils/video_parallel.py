...
from converter.utils.camera_utils import get_mapped_filename, save_camera_info_to_json_new
...
def save_one_color_video_ffmpeg(cam_name, imgs, output_dir, raw_config):
    ...
    try:
        ...
        log_print(f"已保存h264视频: {video_path}")
    except Exception as e:
        log_print(f"ffmpeg编码失败: {e}")
...
def save_one_depth_video_ffmpeg(cam_name, imgs, output_dir, raw_config):
    ...
    try:
        subprocess.run(ffmpeg_cmd, check=True)
        log_print(f"已保存深度视频: {video_path}")
    except Exception as e:
        log_print(f"ffmpeg编码失败: {e}")
...
def save_color_videos_parallel(
    ...
    def save_one_color_video(cam_name, imgs):
        ...
        try:
            ...
            log_print(f"相机 {cam_name} 没有帧，跳过。")
            return
    ...
    #     concurrent.futures.wait(futures)
    log_print("[DEBUG] save_color_videos_parallel start")
    for cam_name, imgs in imgs_per_cam_color.items():
        save_one_color_video(cam_name, imgs)
    log_print("[DEBUG] save_color_videos_parallel end")
...
def save_one_depth_video_16U(cam_name, imgs, output_dir, raw_config):
    ...
    log_print(f"[DEBUG] save_one_depth_video_16U called for {cam_name}, 帧数: {len(imgs)}")
    ...
    if is_hand_camera:
        log_print(f"[DENOISE] {cam_name} 检测为手部相机，将进行深度去噪处理")
    ...
    for idx, img_bytes in enumerate(imgs):
        ...
        if isinstance(img_bytes, bytes):
            ...
            if idx_png == -1:
                log_print(f"[{cam_name}] 第{idx}帧未找到PNG头，跳过")
                continue
            png_data = img_bytes[idx_png:]
        else:
            log_print(f"[{cam_name}] 第{idx}帧数据类型异常，跳过")
            continue
        ...
        if img is None:
            log_print(f"[{cam_name}] 第{idx}帧解码失败，跳过")
            continue
        if img.ndim > 2:
            img = img[:, :, 0]
        ...
        if is_hand_camera:
            try:
                ...
                #     log_print(f"[DENOISE] {cam_name} 第{idx}帧去噪完成")
            except Exception as e:
                log_print(f"[DENOISE] {cam_name} 第{idx}帧去噪失败: {e}")
                # 去噪失败时继续使用原图
    ...
    try:
        subprocess.run(ffmpeg_cmd, check=True)
        log_print(f"[{cam_name}] 已保存16位无损深度视频: {video_path}")
        if is_hand_camera:
            log_print(f"[DENOISE] {cam_name} 去噪处理完成并保存")
    except Exception as e:
        log_print(f"[{cam_name}] ffmpeg编码失败: {e}")
...
def save_depth_videos_16U_parallel(
    ...
    os.makedirs(output_dir, exist_ok=True)
    log_print("[DEBUG] save_depth_videos_16U_parallel start")
    log_print("imgs_per_cam_depth keys:", imgs_per_cam_depth.keys())
    for k, v in imgs_per_cam_depth.items():
        log_print(f"{k}: {len(v)} 帧")
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = []
        for cam_name, imgs in imgs_per_cam_depth.items():
            log_print(f"[DEBUG] 提交 {cam_name} 到线程池, 帧数: {len(imgs)}")
            futures.append(
                executor.submit(
        ...
        concurrent.futures.wait(futures)
    log_print("[DEBUG] save_depth_videos_16U_parallel end")
...
def save_depth_videos_enhanced_parallel(
    ...
    def save_one_enhanced_depth_video(cam_name, depth_imgs):
        ...
        为单个相机保存增强深度视频
        """
        log_print(f"[ENHANCED] 开始处理相机 {cam_name}, 深度帧数: {len(depth_imgs)}")
        ...
        try:
            ...
            if min_frames == 0:
                log_print(f"[ENHANCED] 相机 {cam_name} 没有有效帧，跳过")
                return
            ...
            for idx in range(min_frames):
                try:
                    ...
                    if depth_img is None:
                        log_print(f"[ENHANCED] {cam_name} 第{idx}帧深度解码失败，跳过")
                        continue
                    ...
                    try:
                        ...
                        log_print(f"[ENHANCED] {cam_name} 第{idx}帧深度增强失败: {e}")
                        # 降级为简单的8到16位转换
                    ...
                    if not success:
                        log_print(f"[ENHANCED] {cam_name} 第{idx}帧保存失败")

                except Exception as e:
                    log_print(f"[ENHANCED] {cam_name} 第{idx}帧处理异常: {e}")
                    continue

            # 检查是否有有效帧
            frame_files = [f for f in os.listdir(temp_img_dir) if f.endswith(".png")]
            if not frame_files:
                log_print(f"[ENHANCED] {cam_name} 没有生成有效帧")
                return
            ...
            try:
                ...
                log_print(f"[ENHANCED] 已保存增强16位深度视频: {video_path}")
                if is_hand_camera:
                    log_print(f"[DENOISE] {cam_name} 去噪处理完成并保存")
            except subprocess.CalledProcessError as e:
                log_print(f"[ENHANCED] {cam_name} FFmpeg编码失败: {e.stderr}")
            except Exception as e:
                log_print(f"[ENHANCED] {cam_name} FFmpeg执行异常: {e}")
        except Exception as e:
            log_print(f"[ENHANCED] {cam_name} 处理异常: {e}")
            import traceback
    ...
    # 并行处理每个相机
    # log_print(f"[ENHANCED] 开始并行处理 {len(compressed_group)} 个相机的增强深度视频")
    # with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    #     futures = []
    ...
    # log_print(f"[ENHANCED] 所有增强深度视频处理完成")
    log_print(f"[ENHANCED] 开始串行处理 {len(compressed_group)} 个相机的增强深度视频")
    for cam_name, depth_imgs in compressed_group.items():
