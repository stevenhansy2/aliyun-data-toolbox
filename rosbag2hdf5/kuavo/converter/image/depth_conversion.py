...
def extract_and_save_frames(bag_path, depth_topic_name, color_topic_name, output_dir):
    ...
    log_print(f"从{depth_topic_name}中提取压缩信息...")
    # 正确获取消息数量
    ...
    for (_, depth_msg, *_), (_, color_msg, *_) in tqdm(
        ...
        try:
            ...
            log_print(f"保存滤波图帧 #{index}")
            index += 1
        except Exception as e:
            log_print(f"处理帧时出错: {str(e)}")

    bag.close()
    log_print(f"成功保存 {index} 个帧对")
    return index
...
def main():
    ...
    log_print(f"共提取了深度图{total}帧，耗时: {elapsed_time:.2f}秒")
    log_print(f"平均FPS: {total/elapsed_time:.2f}")

    if total > 0:
        encode_video_with_ffmpeg(depth_temp_dir, output_video_path, framerate=framerate)
    else:
        log_print("No frames extracted. Check topic and bag file.")
