...
class Config:
    ...
    use_streaming_video: bool = True  # 是否使用流式视频写入（降低内存峰值）
    video_queue_limit: int = 300  # 流式写入队列大小限制
    hdf5_batch_size: int = 10000  # HDF5 分批写入的批次大小
    hdf5_large_threshold: int = 10000  # 超过此行数的数据集使用分批写入
    ...
    def default_cameras2topics(self) -> dict:
        ...
        cameras2topics = {
            'head_cam_h': '/head_cam_h/image_raw',
            'wrist_cam_l': '/wrist_cam_l/image_raw',
            'wrist_cam_r': '/wrist_cam_r/image_raw'
        }
        if self.which_arm == 'left':
            return {k: v for k, v in cameras2topics.items() if k != 'wrist_cam_r'}
        elif self.which_arm == 'right':
            return {k: v for k, v in cameras2topics.items() if k != 'wrist_cam_l'}
        elif self.which_arm == 'both':
...
def load_config_from_json(config_path: str) -> Config:
    ...
    for bag in config_dict.get('bags', []):
        ...
        for step in bag_dict.get('steps') or []:
            ...
            steps.append({
                ...
                "english_action_text": step_dict.get('english_action_text', "default_english_action_text"),
                "detailed_description": step_dict.get('detailed_description', "default_detailed_description"),
            })
        ...
        if steps_start is not None and steps_end is not None:
            ...
            log_print(f"[CONFIG] Bag使用steps时间范围: start={steps_start}, end={steps_end} (从 {len(steps)} 个步骤中获取)")
        else:
            ...
            if steps:
                log_print(f"[CONFIG] Bag的steps中未找到有效时间范围，使用原始时间范围: start={original_start}, end={original_end}")
            else:
                log_print(f"[CONFIG] Bag无steps，使用原始时间范围: start={original_start}, end={original_end}")
        
        # 支持每个bag独立的场景/任务等字段
        for field in [
            'scene_name', 'sub_scene_name', 'init_scene_text', 'english_init_scene_text',
            'task_name', 'english_task_name', 'data_type', 'episode_status', 'data_gen_mode',
            'sn_code', 'sn_name', 'task_code','bag_id' ,'length' # <-- 新增'task_code'
        ]:
    ...
    return Config(
        ...
        eef_type=eef_type,
        which_arm=which_arm,
        dex_dof_needed=config_dict.get('dex_dof_needed', 6),
        ...
        english_init_scene_text=config_dict.get('english_init_scene_text', "default_english_init_scene_text"),
        task_name=config_dict.get('task_name', "default_task_name"),
        english_task_name=config_dict.get('english_task_name', "default_english_task_name"),
        ...
        hdf5_export_mask=config_dict.get('hdf5_export_mask', False),  # 是否导出HDF5格式的mask图片
        main_timeline=(
            config_dict.get('main_timeline', {}).get('topic', "/cam_h/color/image_raw/compressed")
