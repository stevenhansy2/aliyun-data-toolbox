...
运行程序为convert_rosbag_to_hdf5.py，request.json 用于配置 rosbag 处理逻辑。
## 2.脚本功能
...
## 3.运行方式
### 读取数据要求：
该脚本通过具有单个或多个 rosbag 与 metadata.json 的特定文件夹。在指定路径输出转化后的数据。
### 运行示例：
```python
python convert_rosbag_to_hdf5.py --bag_dir "./testbag/" --metadata_json_dir "./testbag/metadata.json" --output_dir "./testbag/" --scene "test_scene" --sub_scene "test_sub_scene" --continuous_action "test_continuous_action" --mode "simplified"
```
### 参数说明：
`--bag_dir`: 必须传入。用于指定读取的文件夹。会自动扫描该目录下的 `.bag` 文件，并自动跳过 `.c.bag`。

`--metadata_json_dir`:可选参数。用于指定当前任务的原始信息。例如`"./testbag/metadata.json"`。不指定时，默认读取 `--bag_dir/metadata.json`。
